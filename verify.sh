...
while getopts a:c:g:i:k:n:p:s:v:r: flag
do
    case "${flag}" in
        a) address=${OPTARG};;
...
then
    forge verify-contract --skip-is-verified-check --rpc-url $rpc --guess-constructor-args --num-of-optimizations $optimizations --compiler-version $compiler --chain $chainIdentifier --watch $address $pathToContract
else
    echo constructorSignature: $constructorSignature
    echo constructorArguments: $constructorArguments

    forge verify-contract --skip-is-verified-check --rpc-url $rpc --guess-constructor-args --num-of-optimizations $optimizations --compiler-version $compiler --chain $chainIdentifier --watch --constructor-args $(cast abi-encode "$constructorSignature" $constructorArguments) $address $pathToContract
fi